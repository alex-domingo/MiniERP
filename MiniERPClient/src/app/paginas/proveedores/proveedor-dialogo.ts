...
import { Proveedor, ProveedorPeticion } from '../../nucleo/modelos';
...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ datos ? 'Editar proveedor' : 'Nuevo proveedor' }}</h2>
      ...
      <mat-dialog-content>
        <div class="formulario">
          <mat-form-field>
            ...
            <mat-hint>Se guarda como 1234567-8</mat-hint>
          ...
          <mat-form-field>
            ...
            <input matInput formControlName="contacto" maxlength="120">
            <mat-error>{{ error('contacto') }}</mat-error>
          </mat-form-field>
          <mat-form-field>
...
export class ProveedorDialogo {
  protected readonly datos = inject<Proveedor | null>(MAT_DIALOG_DATA);
  private readonly api = inject(CatalogoApi);
  private readonly ref = inject(MatDialogRef<ProveedorDialogo, Proveedor>);
  ...
  protected readonly formulario = inject(FormBuilder).nonNullable.group({
    ...
    contacto: [this.datos?.contacto ?? '', [Validators.maxLength(120)]],
  ...
  protected guardar(): void {
    ...
    const valores = limpiar(this.formulario.getRawValue()) as ProveedorPeticion;
    const peticion = this.datos
      ? this.api.proveedores.actualizar(this.datos.idProveedor, valores)
      : this.api.proveedores.crear(valores);
    peticion.subscribe({
